...
import { Question, QuestionType } from '../reading/entities/question.entity';
import {
  TestAttempt,
  AttemptStatus,
} from '../reading/entities/test-attempt.entity';
import { Answer } from '../reading/entities/answer.entity';
import { scoreToBand } from '../reading/utils/band-score.util';
...
export class ListeningService {
  ...
  async listTests() {
    const tests = await this.testRepo.find({
      where: { moduleType: ModuleType.LISTENING, isPublished: true },
  ...
  async submitAttempt(attemptId: string, userId: string) {
    ...
    await this.studentsService.updateListeningBand(userId, bandScore);
    ...
    if (priorCompletedCount === 0) {
      await this.notificationsService.create(
        ...
        'First Listening test completed! 🎉',
        'You completed your first Listening practice test. Keep it up!',
      );
    }
